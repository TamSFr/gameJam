...
function gameU(){
	...
	if (xAxis2 < -0.1 || cursors.left.isDown){
		player1.body.velocity.x = -vitesse1;
	}
	
	//if (cursors.right.isDown){ 
	if (xAxis2 > 0.1 || cursors.right.isDown){
		player1.body.velocity.x = vitesse1;
	} 
	
	//if (cursors.up.isDown){ 
	if (yAxis2 < -0.1 || cursors.up.isDown){
		player1.body.velocity.y = -vitesse1;
	}
	
	//if (cursors.down.isDown){ 
	if (yAxis2 > 0.1 || cursors.down.isDown){
		player1.body.velocity.y = vitesse1;
	} 
	...
	//if (q_key.isDown){ 
	if (xAxis < -0.1 || q_key.isDown){
		player2.body.velocity.x = -vitesse2;
	}
	
	if (xAxis > 0.1 || d_key.isDown){ 
		player2.body.velocity.x = vitesse2;
	} 
	if (yAxis < -0.1 || z_key.isDown){ 
		player2.body.velocity.y = -vitesse2;
	}
	
	if (yAxis > 0.1 || s_key.isDown){ 
		player2.body.velocity.y = vitesse2;
	} 
	...
	if(varJ == 1){
		//Controle du player 2
		//if (g_key.isDown){ 
		if (pad1.isDown(PAD_A)|| g_key.isDown) {
			varJ = 0;
		...
		if (pad1.isDown(PAD_B) || h_key.isDown){ 
			if(vitesse2 < 600){
				vitesse2 += 100;
				varJ = 0;
				game.time.events.add(tJour, jour1, this);
		...
		if (pad1.isDown(PAD_X) || j_key.isDown){ 
			vie1=vie1+1;
			varJ = 0;
			game.time.events.add(tJour, jour1, this);
			menuF2.loadTexture('pim');
	...
	if(varJ2 == 1){
		...
		//if (un_key.isDown){ 
		if (pad2.isDown(PAD_A) || un_key.isDown) {
			varJ2 = 0;
			creaNoiCoco1(3*grille,7*grille);
		...
		if (pad2.isDown(PAD_B) || deux_key.isDown){ 
			if(vitesse1 < 600){
				vitesse1 += 100;
				varJ2 = 0;
				game.time.events.add(tJour, jour2, this);
		...
		if (pad2.isDown(PAD_X) || trois_key.isDown){ 
			vie2=vie2+1;
			varJ2 = 0;
			game.time.events.add(tJour, jour2, this);
			menuF.loadTexture('pim');
